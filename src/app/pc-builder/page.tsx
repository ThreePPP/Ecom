...
const COMPONENT_TYPES: ComponentType[] = [
  ...
  { id: "memory", name: "แรม", icon: "/icons/ram.png", category: "Memory" },
  {
    ...
    category: "Storage",
  }, // Added for completeness if needed, but user asked for specific list. I will stick to user list but maybe add storage if it makes sense? User said: CPU,CPU Cooler,Mainboard,Memory,Power Supply,Case. I will stick to that.
...
const ACTIVE_COMPONENTS = [
  ...
  { id: "memory", name: "แรม", icon: "/icons/ram.png", category: "Memory" },
  {
    ...
    icon: "/icons/harddisk.png",
    category: "Storage",
  }, // I don't have icon for storage, will check or use generic.
  {
    id: "cooler",
    name: "ชุดน้ำ/พัดลม",
    icon: "/icons/cpu.png",
...
export default function PCBuilderPage() {
  ...
  const handleSelectProduct = (product: Product) => {
    ...
    const currentIndex = COMPONENT_LIST.findIndex(
      (c) => c.id === activeComponentId,
    );
    ...
    const matchesFilters = Object.entries(selectedFilters).every(
      ([key, values]) => {
        ...
        if (key === "sockets") {
          ...
          return values.some(
            (v) =>
              (socket && socket.toLowerCase() === v.toLowerCase()) ||
              product.name.toLowerCase().includes(v.toLowerCase()),
          );
        }
          ...
          return values.some((v) => {
            ...
            if (match) {
              ...
              return subModels.some((sub) => {
                ...
                return (
                  (chipset && chipset.toLowerCase().includes(subLower)) ||
            ...
            return (
              (chipset && chipset.toLowerCase().includes(vLower)) ||
              product.name.toLowerCase().includes(vLower)
            );
          });
        }
        ...
        if (key === "series") {
          ...
          if (
            activeComponentId === "gpu" ||
            (activeComponent &&
              activeComponent.category.toLowerCase() === "vga")
          ) {
            return values.some((v) => {
              const vLower = v.toLowerCase();
              // ตรงกับ series
          ...
          return values.some((v) => {
            ...
            if (series && series.toLowerCase().includes(vLower)) return true;
            if (nameLower.includes(vLower)) return true;
            if (vLower.includes("core i5") && nameLower.includes("i5"))
              return true;
            if (vLower.includes("core i3") && nameLower.includes("i3"))
              return true;
            if (vLower.includes("core i7") && nameLower.includes("i7"))
              return true;
            if (vLower.includes("core i9") && nameLower.includes("i9"))
              return true;
            if (vLower.includes("ryzen 3") && nameLower.includes("ryzen 3"))
              return true;
            if (vLower.includes("ryzen 5") && nameLower.includes("ryzen 5"))
              return true;
            if (vLower.includes("ryzen 7") && nameLower.includes("ryzen 7"))
              return true;
            if (vLower.includes("ultra 5") && nameLower.includes("ultra 5"))
              return true;
            if (vLower.includes("ultra 7") && nameLower.includes("ultra 7"))
              return true;
            if (vLower.includes("ultra 9") && nameLower.includes("ultra 9"))
              return true;
            return false;
          });
        }
        ...
        if (key === "sizes") {
          ...
          return values.some((v) => {
            ...
            if (sizeMatch) {
              ...
              if (
                (size && regex.test(size.toLowerCase())) ||
            ...
            return (
              (size && size.toLowerCase().includes(vLower)) ||
              product.name.toLowerCase().includes(vLower)
            );
          });
        }
        ...
        if (key === "bus") {
          ...
          return values.some((v) => {
            ...
            if (speedMatch) {
              ...
              if (
                (bus && bus.toLowerCase().includes(speed)) ||
                product.name.toLowerCase().includes(speed)
              )
                return true;
            }
            // fallback: match ทั้ง string เดิม
            return (
              (bus && bus.toLowerCase().includes(vLower)) ||
              product.name.toLowerCase().includes(vLower)
            );
          });
        }
        ...
        if (key === "capacities") {
          ...
          return values.some(
            (v) =>
              (capacity && capacity.toLowerCase().includes(v.toLowerCase())) ||
              product.name.toLowerCase().includes(v.toLowerCase()),
          );
        }
        ...
        if (key === "power") {
          ...
          return values.some((v) => {
            ...
            if (wattMatch) {
              ...
              if ((power && regex.test(power)) || regex.test(product.name))
                return true;
            }
            // fallback: match the original string
            return (
              (power && power.toLowerCase().includes(vLower)) ||
              product.name.toLowerCase().includes(vLower)
            );
          });
        }
        ...
        if (key === "mainboardSupport") {
          ...
          return values.some((v) => {
            ...
            const supportNorm = support
              ? support.toLowerCase().replace(/[-\s]/g, "")
              : "";
            const nameNorm = product.name.toLowerCase().replace(/[-\s]/g, "");
            if (
              (supportNorm && regex.test(supportNorm)) ||
              regex.test(nameNorm)
            )
              return true;
            // fallback: match original string (for partials)
            return (
              (support && support.toLowerCase().includes(vLower)) ||
              product.name.toLowerCase().includes(vLower)
            );
          });
        }
                            ...
                            <div className="flex justify-between items-end mt-2">
                              ...
                              >
                                ...
                                {animatingProductId === product._id && (
                                  <>
                                    <span
                                      className="absolute top-0 left-1/2 w-1 h-1 bg-yellow-300 rounded-full animate-bounce"
                                      style={{ animationDelay: "0ms" }}
                                    ></span>
                                    <span
                                      className="absolute bottom-0 left-1/4 w-1 h-1 bg-green-300 rounded-full animate-bounce"
                                      style={{ animationDelay: "100ms" }}
                                    ></span>
                                    <span
                                      className="absolute top-1/2 right-0 w-1 h-1 bg-blue-300 rounded-full animate-bounce"
